def help_info(version):
    ...
    print('win环境变量索引,可以匹配环境变量内的程序')
    print('输入##会进入执行模式(将输入作为代码执行)')
    print('以#起始将会被判断为指令')
    print('可用的指令:')
    print('#help')
    print('#cmd [code],#bash [code]执行cmd或者basg命令')
    ...
    print('#net up(update) [-cover] 不覆盖的更新,带选项为覆盖 | net upmain [-cover](更新主程序) | net uptools [-cover](更新模块) | getv(get_version_list) 获取模块版本列表文件')
