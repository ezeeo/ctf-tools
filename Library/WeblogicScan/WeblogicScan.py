 ...
 \ \ /\ / / _ \ '_ \| |/ _ \ / _` | |/ __| \___ \ / __/ _` | '_ \ 
  \ V  V /  __/ |_) | | (_) | (_| | | (__   ___) | (_| (_| | | | |
   \_/\_/ \___|_.__/|_|\___/ \__, |_|\___| |____/ \___\__,_|_| |_|
                             |___/ 
                             By Tide_RabbitMask | V {} --modify
'''.format(version)

def PocS(rip,rport,use_https=False):
    if use_https:
        h='https://'
    else:
        h='http://'
    ...
    try:
        poc.CVE_2017_3506.run(rip, rport, 0,h)
    except:
        print ("[-]CVE_2017_3506 not detected.")

    print('[*]CVE_2017_10271 is testing...')
    try:
        poc.CVE_2017_10271.run(rip, rport, 0,h)
    except:
...
def run():
    ...
    if len(sys.argv)<3:
        ...
        if len(sys.argv)==3:
            PocS(url,port,False)
        elif sys.argv[3] in ('Y','y','https','HTTPS'):
            PocS(url,port,True)
        else:
            PocS(url,port,False)
